# -*- coding: utf-8 -*-
"""
gui.py  —  NAO Robot GUI Control Center
All operations are centered around this interface:
  • Configure problem (map, orders, obstacles)
  • Run MGA (Modified Genetic Algorithm with D* Lite)
  • Simulation (animation of robot movement on map)
  • Run on real NAO (NAOqi / qi-framework, with FAKE mode)
  • Export MGA evaluation charts
...
# Default map size (customizable via GUI)
MAP_W_DEFAULT = 100
MAP_H_DEFAULT = 100
...
def generate_random_obstacles(n_blocks, map_w=None, map_h=None,
    ...
    """
    Generate n_blocks obstacles as 1-cell-thick straight lines on a map_w x map_h grid.
    Each obstacle is a horizontal (height=1) or vertical (width=1) random segment.
    """
...
def random_free_point(grid_inflated, rng, exclude=None, max_tries=5000):
    """Find a random free cell on the grid (infers size from grid)."""
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("NAO Delivery — MGA | Control Center")
        ...
        self._replan_count = 0    # number of replans during simulation
    ...
    def _build_ui(self):
        ...
        self.status_var = tk.StringVar(value="Ready. Please create an environment.")
    ...
    def _build_tab_problem(self, p):
        ...
        # --- Obstacle coverage statistics ---
        frm_stats = ttk.LabelFrame(p, text="Coverage statistics", padding=6)
        frm_stats.pack(fill=tk.X, pady=(8, 0))
        self.var_occupancy = tk.StringVar(value="Obstacle occupancy:  —")
        ttk.Label(frm_stats, textvariable=self.var_occupancy,
                  font=("Consolas", 9)).pack(anchor=tk.W)
        self.var_inflation = tk.StringVar(value="C-space inflation:  —")
        ttk.Label(frm_stats, textvariable=self.var_inflation,
                  font=("Consolas", 9)).pack(anchor=tk.W, pady=(3, 0))
        ...
        ttk.Spinbox(p, from_=10, to=500, textvariable=self.var_pop,
                    width=10).pack(anchor=tk.W)
        ttk.Label(p, text="Max generations:").pack(anchor=tk.W, pady=(6, 0))
        ...
        self.ga_btn = ttk.Button(p, text="① Run MGA",
                                 command=self.action_run_ga)
        self.ga_btn.pack(fill=tk.X, pady=2)
        # D* Lite button hidden - runs automatically after GA
        self.astar_btn = ttk.Button(p, text="② Step 2: Run D* Lite",
                                    command=self.action_run_astar,
                                    state=tk.DISABLED)
        # astar_btn.pack(fill=tk.X, pady=2)  # Hidden
    ...
    def _build_tab_sim(self, p):
        ttk.Label(p, text="ROBOT SIMULATION",
                  font=("Segoe UI", 10, "bold")).pack(anchor=tk.W)
        ttk.Label(p, text="Robot moves along the planned path on the map.\n"
                          "Run MGA first to generate the path.",
                  foreground="#555", justify=tk.LEFT,
                  wraplength=290).pack(anchor=tk.W, pady=(2, 8))

        ttk.Label(p, text="Speed (cells/second):").pack(anchor=tk.W)
        ...
        self.sim_speed_lbl = ttk.Label(p, text="20.0 cells/s")
        ...
        ttk.Label(p, text="DYNAMIC OBSTACLES",
                  font=("Segoe UI", 10, "bold")).pack(anchor=tk.W)
        ttk.Label(p, text="Obstacles appear randomly during simulation.\n"
                          "Robot auto-replans when path is blocked.",
                  foreground="#555", justify=tk.LEFT,
    ...
    def _build_tab_nao(self, p):
        ...
        ttk.Checkbutton(p, text="Force FAKE mode (only print commands)",
        ...
        self.nao_run_btn = ttk.Button(p, text="🚶  Run on D* Lite path",
        ...
        ttk.Separator(p).pack(fill=tk.X, pady=6)
        ttk.Label(p, text="NAO Log:").pack(anchor=tk.W)
    ...
    def _build_tab_eval(self, p):
        ...
        ttk.Separator(p).pack(fill=tk.X, pady=8)
        self.var_eval_random = tk.BooleanVar(value=True)
        ttk.Checkbutton(p, text="Random mode (different each run)",
                        variable=self.var_eval_random).pack(anchor=tk.W, pady=(2, 4))
        ttk.Separator(p).pack(fill=tk.X, pady=4)
        ttk.Label(p, text="Chart list:").pack(anchor=tk.W)
    ...
    def _apply_grid_ticks(self, ax):
        ...
        # Minor ticks: every 1 cell (only draw when map <= 150)
        if w <= 150 and h <= 150:
    ...
    def _draw_environment(self):
        ...
        if self.full_path:
            smooth = smooth_path(self.full_path)
            xs = [p[0] + 0.5 for p in smooth]
            ys = [p[1] + 0.5 for p in smooth]
            self.ax.plot(xs, ys, "-", color="#2ca02c", lw=1.8, alpha=0.9,
                         label="D* Lite path", zorder=3)
        ...
        title = (f"Map {self.map_w}×{self.map_h} | {len(self.orders)} orders | "
                 f"Occ: {self.wall_pct:.1f}%  Infl: {self.gray_pct:.1f}% | "
                 f"○ pickup, □ dropoff")
        if self.full_path is not None:
            ...
            title += (f"\n[MGA partial] GA straight-line reference: "
                      f"{self.result.total_distance:.2f} cells  (D* Lite not run)")
        self.ax.set_title(title, fontsize=11, y=1.02)
    ...
    def action_generate(self):
        ...
        if n_pick != n_drop:
            messagebox.showwarning("Warning",
                f"Pickup count ({n_pick}) differs from dropoff ({n_drop}). "
        ...
        wall_cells = sum(1 for y in range(self.map_h) for x in range(self.map_w)
                         if self.grid_raw[y][x] == 1)
        gray_cells = sum(1 for y in range(self.map_h) for x in range(self.map_w)
                         if self.grid_inflated[y][x] == 1 and self.grid_raw[y][x] == 0)
        self.wall_pct = wall_cells / total_cells * 100.0
        self.gray_pct = gray_cells / total_cells * 100.0
        self.start = (0, 0)
        try:
            used = {self.start}
            pickups = []
        ...
        # astar_btn state no longer needed (hidden)
        self._update_sim_buttons()
        self._update_nao_run_button()
        self.var_occupancy.set(
            f"Obstacle occupancy:   {wall_cells:>5} cells  ({self.wall_pct:.2f}%)")
        self.var_inflation.set(
            f"C-space inflation:   {gray_cells:>5} cells  ({self.gray_pct:.2f}%)")
        self.status_var.set(
            f"Created environment {self.map_w}×{self.map_h}:\n"
            f"  • {n_orders} orders,  {n_obs} obstacle blocks\n"
            f"  • Obstacle occupancy: {wall_cells} cells ({self.wall_pct:.2f}%)\n"
            f"  • C-space inflation:  {gray_cells} cells ({self.gray_pct:.2f}%)\n"
            f"  • Start = (0, 0)\n\n"
            f"→ Go to « Problem » tab to run GA, "
            f"or « Simulation » after D* Lite path is ready.")
    ...
    def action_run_ga(self):
        ...
        self.status_var.set("⏳ Running MGA...")
    ...
    def _on_ga_and_astar_done(self):
        ...
        print("\n" + "="*60)
        print("MGA (Modified Genetic Algorithm) - Results")
        print("="*60)
        print(f"Map Size: {self.map_w} x {self.map_h}")
        print(f"Orders: {len(self.orders)}")
        print(f"Obstacle Occupancy: {self.wall_pct:.2f}%")
        print(f"C-space Inflation: {self.gray_pct:.2f}%")
        print("-"*60)
        print(f"GA Octile Distance: {self.result.total_distance:.2f} cells")
        print(f"GA Generations: {self.result.generations_run}")
        print(f"Actual Distance (MGA): {self.actual_distance:.2f} cells")
        print(f"Total Steps: {len(self.full_path)}")
        print(f"Difference vs GA: {sign}{gain:.2f} cells")
        print("-"*60)
        ...
        self.status_var.set(
            f"✓ MGA complete\n"
            f"  • GA Octile: {self.result.total_distance:.2f} cells\n"
            f"  • Generations: {self.result.generations_run}\n"
            f"  • Actual distance: {self.actual_distance:.2f} cells\n"
            f"  • Steps: {len(self.full_path)}\n"
            f"  • Δ vs GA: {sign}{gain:.2f} cells\n"
            f"  • Order: {order_str}\n\n"
            f"→ You can now SIMULATE or RUN ON REAL NAO.")
        self._update_sim_buttons(); self._update_nao_run_button()
        self._draw_environment()
        
        # Auto-generate fitness chart
        self._generate_fitness_chart()

    def _generate_fitness_chart(self):
        """Generate and save the fitness f(x) chart after MGA completes."""
        try:
            import os
            os.makedirs("charts", exist_ok=True)
            
            # Get history from result
            best_history = self.result.history
            mean_history = self.result.history_mean
            
            # Convert distance to fitness with gradual scaling (smoother curve)
            # Use square root scaling for more gradual, smoother slope
            max_dist = best_history[0] if best_history else 1
            min_dist = best_history[-1] if best_history else 1
            
            # Normalize to 0-100 range with square root for gradual curve
            fitness_best = [100 * (1 - ((d - min_dist) / (max_dist - min_dist + 1e-6)) ** 0.5) 
                           for d in best_history]
            fitness_mean = [100 * (1 - ((d - min_dist) / (max_dist - min_dist + 1e-6)) ** 0.5) 
                           for d in mean_history]
            
            gens = range(1, len(fitness_best) + 1)
            ...
            axes[0].text(len(gens) * 0.05, fitness_best[-1] * 1.02,
            ...
            axes[0].set_title("f(x) của cá thể tốt nhất\n(f(x) càng cao = càng tối ưu)", 
            ...
            axes[1].axhline(y=fitness_mean[-1], color="#2ca02c", 
                           linestyle="--", lw=1.5, alpha=0.7)
            axes[1].text(len(gens) * 0.05, fitness_mean[-1] * 1.02,
            ...
            axes[1].set_title("Mean f(x) của quần thể\n(f(x) càng cao = càng tối ưu)", 
                              fontsize=12, fontweight="bold")
            axes[1].grid(alpha=0.3)
            
            fig.suptitle(f"MGA Fitness - {len(self.orders)} orders, {self.result.generations_run} generations",
                         fontsize=13, fontweight="bold", y=0.98)
            fig.tight_layout(rect=[0, 0, 1, 0.95])
            
            # Save chart
            chart_path = os.path.join("charts", "ga_fitness_vs_generations.png")
            ...
            print(f"[Warning] Could not generate fitness chart: {e}")
    ...
    def action_run_astar(self):
        ...
        self.status_var.set("⏳ Step 2: D* Lite finding obstacle-free path...")
    ...
    def _run_astar_thread(self):
        try:
            ...
            err = str(e)
            self.root.after(0, lambda: self._on_error(err, step="D* Lite"))
    ...
    def _on_error(self, msg, step="GA"):
        self.ga_btn.configure(state=tk.NORMAL)
        self.status_var.set(f"Error in step {step}: {msg}")
    ...
    def action_sim_play(self):
        if not self.full_path:
            messagebox.showinfo("Info", "No D* Lite path available yet."); return
        ...
        self.dyn_mgr.clear()
        self._sim_full_path = list(self.full_path)  # copy for replan modifications
        ...
        # Compute remaining waypoints for replanning
    ...
    def _sim_step(self):
        ...
        if self.var_dyn_enabled.get():
            ...
            # Spawn new obstacle ON PATH according to frequency
            if self.sim_index > 0 and self.sim_index % freq == 0:
                # Avoid placing on waypoints (pickup/dropoff)
                avoid = {self.start}
            ...
            if blocked_idx is not None:
                # --- 3) Replan: recalculate from current position ---
                ...
                if new_path is not None:
                    ...
                    # Combine traversed portion + new path
                    self._sim_full_path = (
                        self._sim_full_path[:self.sim_index] + new_path)
                    self.sim_progress.configure(
                        maximum=len(self._sim_full_path) - 1)
                    self.sim_msg_var.set(
                        f"⚠ Dynamic obstacle! Replanning "
                        f"(time {self._replan_count})")
                    # Redraw new path on map
                    self._redraw_path()
                # If no path found → continue walking, obstacle may expire
        ...
        if (x, y) in self._waypoint_set and self.sim_index > 0:
            ...
            if 1 <= wp_idx <= 2 * len(self.orders):
                ...
                # Update remaining waypoints when arriving at a waypoint
    ...
    def _update_remaining_waypoints(self, current_pos):
        """Remove already-passed waypoints from the remaining list."""
    ...
    def _draw_dynamic_obstacles(self):
        """Draw/update dynamic obstacles (red cells) on the map."""
        if self._dyn_scatter is not None:
            self._dyn_scatter.remove()
            self._dyn_scatter = None
    ...
    def _redraw_path(self):
        """Redraw the path (after replan) on the map with smooth curves."""
        # Remove old path and redraw
        for line in self.ax.lines[:]:
            if hasattr(line, 'get_label') and line.get_label() == "D* Lite path":
                line.remove()
        # Draw new smoothed path
        smooth = smooth_path(self._sim_full_path)
        xs = [p[0] + 0.5 for p in smooth]
        ys = [p[1] + 0.5 for p in smooth]
        self.ax.plot(xs, ys, "-", color="#2ca02c", lw=1.8, alpha=0.9,
                     label="D* Lite path", zorder=3)
    ...
    def action_nao_disconnect(self):
        if self.nao_running:
            ...
            self._log_nao("[stop] please stop before disconnecting...")
            return
        self.nao.disconnect()
    ...
    def action_nao_run(self):
        if not (self.nao.connected and self.full_path):
            messagebox.showinfo("NAO", "Not ready (connect + D* Lite required)."); return
    ...
    def _on_nao_step(self, i, n, cell):
        ...
        if cell in self._waypoint_set and i > 0:
            ...
            if 1 <= wp_idx <= 2 * len(self.orders):
                ...
                label = self.orders[task_order_id(t)].label
                if task_is_pickup(t):
                    self.nao.say(f"Picked up {label}")
                    self._log_nao(f"  ► P{label}  (cell {cell})")
                else:
                    self.nao.say(f"Delivered {label}")
    ...
    def action_eval_run(self):
        ...
        tag = "random" if rand else "deterministic"
    ...
    def _on_eval_done(self, err):
        ...
        if err:
            self.status_var.set(f"Error exporting charts: {err}")
